	...
	if !(_Name in _Filter) then 
	{
		...
		_Laptop addAction [_NameC,
		{
			...
			if (_Available) then 
			{
				...
				_BaseZValues = _Positions apply { _x#2 };
				For "_y" from 0 to 2 step 1 do
				{
					For "_i" from 0 to 7 step 1 do 
					{
						_Pos = _Positions#_i;
						_Z = (_BaseZValues#_i) + (_Height * _y);
						_Pos set [2,_Z];
						_Light = CreateVehicle [_SphereClass, _Pos, [], 0, "CAN_COLLIDE"];
						_Light setVectorUp surfaceNormal getPosASL _Light;
