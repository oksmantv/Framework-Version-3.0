...
switch (_case) do {
	...
	case 1: {
		...
		{[_x] remoteExec ["GW_SetDifficulty_fnc_setSkill",3]} foreach units _Group;
		[StaticTrigger,0,EAST] spawn OKS_fnc_Populate_StaticWeapons;
	...
	case 2: {
		...
		[Trigger_1,false,[14,15,true,false],east,0,0,0,[0,true,false,0],[0,false],[1,false],[0,0,0,0,0],false] spawn OKS_fnc_CreateZone;
		[Trigger_2,false,[12,20,false,false],east,0,0,0,[0,true,false,0],[0,false],[1,false],[0,0,0,0,0],false] spawn OKS_fnc_CreateZone;
	};

	case 3: {
		[Trigger_3,false,[24,25,true,false],east,0,0,0,[0,true,false,0],[0,false],[1,false],[0,0,0,0,0],false] spawn OKS_fnc_CreateZone;
	};

	case 4: {

		// Beach strike 1
		[arty_1,[getPos target_13,getPos target_14,getPos target_15,getPos target_16],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression; sleep 2;
		[arty_2,[getPos target_9,getPos target_10,getPos target_11,getPos target_12],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
		[arty_3,[getPos target_1,getPos target_2,getPos target_3,getPos target_4],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
		[arty_4,[getPos target_5,getPos target_6,getPos target_7,getPos target_8],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;
	
		["ANS Thorax","side","Missile strike inbound on beach defences. Assault the beach, good luck!"] remoteExec ["OKS_fnc_Chat",0];
	};
	...
	case 5: {
		...
		if(((Alive nest_1) && !isNull (gunner nest_1) || (Alive nest_2) && !isNull (gunner nest_2))) then {
			...
			// Beach strike 2
			[arty_3,[getPos zulu_1,getPos zulu_2],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
			[arty_4,[getPos zulu_3,getPos zulu_4],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
			[arty_2,[getPos zulu_5,getPos zulu_6],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
			[arty_1,[getPos zulu_7,getPos zulu_8],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;

			sleep 10;
			["ANS Thorax","side","Additional Missile strike inbound on Point Zulu. Take cover, take cover, take cover!"] remoteExec ["OKS_fnc_Chat",0];

			{
				group _X setBehaviour "COMBAT";
				_x setVehicleAmmo 1;
			} foreach [turret_4, turret_2, turret_3, turret_1];
		};
	};
	...
	case 6: {
		...
		// Point Bear Strike
		[arty_3,[getPos bear_1,getPos bear_2,getPos bear_3],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
		[arty_4,[getPos bear_4,getPos bear_5,getPos bear_6],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
		[arty_2,[getPos bear_7,getPos bear_8,getPos bear_9],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;  sleep 2;
		[arty_1,[getPos bear_10,getPos bear_11,getPos bear_12],independent,1,0,true,false, 0, false] spawn OKS_fnc_ArtySuppression;

		sleep 5; ["ANS Thorax","side","Additional Missile strike inbound on Point Bear. Once complete you may approach your next objective!"] remoteExec ["OKS_fnc_Chat",0];
	};
