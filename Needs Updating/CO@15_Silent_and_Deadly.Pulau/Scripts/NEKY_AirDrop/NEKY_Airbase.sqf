...
While {Alive _Object && _AirbaseRespawnCount > 0 } do {
	...
	if({([_ReinforcementZone, _x] call BIS_fnc_inTrigger) && (_OKS_Side knowsAbout _X > 3.98 || _OKS_Side knowsAbout vehicle _X > 3.98) && (isTouchingGround (vehicle _X))} count AllPlayers > 0) then {
		_Delay = (_AirbaseRefreshRate + (random _AirbaseRefreshRate));
		sleep _Delay;
		systemChat format["Players detected.. delay %1",_Delay];
	};
	{
		if (([_ReinforcementZone, _x] call BIS_fnc_inTrigger) && (_OKS_Side knowsAbout _X > 3.98 || _OKS_Side knowsAbout vehicle _X > 3.98) && (isTouchingGround (vehicle _X)))
		then
		{
