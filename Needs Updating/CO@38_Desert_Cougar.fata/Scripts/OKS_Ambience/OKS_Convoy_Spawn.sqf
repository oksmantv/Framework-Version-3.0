...
private _WaitUntilCombat = {
    ...
    if(((!isNull gunner _Vehicle) || (_Vehicle emptyPositions "gunner"> 0)) || ((!isNull commander _Vehicle) || (_Vehicle emptyPositions "commander"> 0))) then {
		...
		if(!isNil "NEKY_Hunt_Run") then {
			if(_Debug_Variable) then {systemChat format ["NEKY Hunt Applied to %1 in %2 - %3",_Crew,_Vehicle,[configFile >> "CfgVehicles" >> typeOf _Vehicle] call BIS_fnc_displayName]};
			
			_HuntTrigger = createTrigger ["EmptyDetector", getPos _Vehicle];
			_HuntTrigger setTriggerArea [1000, 1000, 0, false];
			_HuntTrigger setTriggerActivation ["ANYPLAYER", "PRESENT", true];
			[_Crew, nil, _HuntTrigger] Spawn NEKY_Hunt_Run;
		}
		else
		{
...
For "_i" from 1 to _Count do {
    ...
    Private _Position = _End getPos [35+(random 25),(random 360)];
