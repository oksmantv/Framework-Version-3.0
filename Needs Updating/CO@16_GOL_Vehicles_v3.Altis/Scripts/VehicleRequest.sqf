...
_Filter = [];	// To avoid having each scheme of same vehicles appear.
{
	...
	if !(_Name in _Filter) then 
	{
		...
		_Laptop addAction [_NameC,
		{
			...
			While { !_Available && _Loop <= 6 } do
			{
				// Check for space
				_Near = _SpawnPos NearObjects ["ALL", 2];
				_Available = Count _Near == 0;
				
				if !(_Available) then 
				{
					// Shift SpawnPos 1m
					_SpawnPos = _SpawnPos getPos [1, _Dir];
			...
			if (_Available) then 
			{
				...
				// Creation of Vehicle
				_Spawned = CreateVehicle [_Veh, _SpawnPos, [], 0, "NONE"];
...
_Laptop AddAction ["<t color='#FF0000'>Clear Area of vehicles 10m</t>",
{
	...
	_Near = _SpawnPos NearObjects 10;
	_Near = _Near select 
	{
		_V = _x;
		!isPlayer _V 
