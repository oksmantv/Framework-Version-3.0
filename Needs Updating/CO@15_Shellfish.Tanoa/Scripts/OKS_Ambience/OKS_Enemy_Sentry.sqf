...
*/
	...
	if(Alive _Unit && [_Unit] call ace_common_fnc_isAwake) then {
		...
		if(_RequiresRadioToCallHunt && !(_Unit getVariable ["GOL_HasRadio",false])) then {
			_NearFriendliesWithRadio = (_Unit nearEntities ["Man",120]) select {!isPlayer _X && side _X == _Side && (_X getVariable ["GOL_HasRadio",false] isEqualTo true) && Alive _X && [_X] call ace_common_fnc_isAwake};
