...
if ((_Units Select 0) > 0) then
{
	...
	if !(isNil "NEKY_Hunt_Run" OR isNil "lambs_wp_fnc_taskHunt") then
	{	
		if !(isNil "lambs_wp_fnc_taskHunt") then {
			{ if ( {Alive _x} count (Units _x) != 0) then { [_X, 1000, 60, [], [], false] spawn lambs_wp_fnc_taskHunt} } forEach _Groups;
		} else {
			if !(isNil _OKS_Zone) then {
