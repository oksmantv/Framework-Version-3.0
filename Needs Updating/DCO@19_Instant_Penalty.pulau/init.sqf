...
Sleep 5;
	...
	*/
	...
	if(GOL_OKS_MHQ_PARADROP isEqualTo 1) then {
		...
		private _Height = 200;      /* Altitude in meters where parachute is opened Upmatically */
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
		...
		*/
		...
		if(isServer) then {
			[officer_1,0.5,30,true,true] execVM "Scripts\OKS_Ambience\OKS_Surrender.sqf";
			/* Example of Dynamic Scripts */
			[Trigger_1,false,[5,12,true,false],east,0,0,0,[0,true,false,0],[0,false],[0,false],[0,0,0,0,0],false] spawn OKS_CreateZone;
			[Trigger_2,false,[5,12,true,false],east,0,0,0,[0,true,false,0],[0,false],[0,false],[0,0,0,0,0],false] spawn OKS_CreateZone;
			[Trigger_3,false,[0,20,false],east,3,0,0,[0,true,false,0],[0,false],[0,false],[0,0,0,0,0],true] spawn OKS_CreateZone;

			[[[[9095.79,9181.41,0.0503922],186,"Up",[]],[[9104.62,9186.44,-0.044939],188,"Up",[]],[[9109.59,9176.28,-0.000267029],186,"Up",[]]],[],[]] call GW_Common_fnc_spawnGroup;
		};
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
		...
		*/
			...
			if(isServer && GOL_OKS_AMBIENCE isEqualTo 1) then {
				...
				[east,"asian"] spawn OKS_FaceSwap;
				[civilian,"asian"] spawn OKS_FaceSwap;
			};
		/*
		*/
