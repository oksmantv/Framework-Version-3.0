...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
		...
		*/
			{[_X,0.5,50,true,true] spawn OKS_Surrender} foreach [officer_1,officer_2,officer_3,officer_4];
			if(isServer && GOL_OKS_AMBIENCE isEqualTo 1) then {
				waitUntil {sleep 1; !(isNil "OKS_FaceSwap")};
				[east,"middleeast"] spawn OKS_FaceSwap;
			};
			if(isServer) then {
				execVM "SoundLoop.sqf";	

				[alarm_1,2600] execVM "Scripts\OKS_Ambience\OKS_AlarmSound.sqf";
				[alarm_2,3000] execVM "Scripts\OKS_Ambience\OKS_AlarmSound.sqf";
			};
