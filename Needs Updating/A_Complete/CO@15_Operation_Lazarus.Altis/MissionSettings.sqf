...
GOL_OPTICS = 0; 		  // Allows sights to be selected from the Arsenal.
GOL_WEAPONS = 0;		  // Allows weapon variations to be selected from Arsenal (If Faction is setup for it).
GOL_ARSENAL_ALLOWED = 0;  // Allows Attachment Menu
...
OKS_Suppression = 0; // Adds suppression sensitivty to AI.
...
[] spawn {
	if(isServer && GOL_OKS_AMBIENCE isEqualTo 1) then {
		...
		[east,"american"] spawn OKS_FaceSwap;
		[independent,"english"] spawn OKS_FaceSwap;
	};
};
