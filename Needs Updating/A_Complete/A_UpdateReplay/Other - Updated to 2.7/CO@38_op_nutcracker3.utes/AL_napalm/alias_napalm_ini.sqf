...
while {ib < number_of_bombs} do 
{
	...
	[_nap_obj_princ,_bombardier_x] spawn 
	{
		...
		[_nap_obj_b,300,60,10,0.5,true,10] execVM "AL_fire\al_fire_deco.sqf";
		[[_nap_obj_b,_dir_x,_dir_y],"AL_napalm\alias_napalm_effect.sqf"] remoteExec ["execVM"];
		_nr_bat = floor (3+random 7);
		[[_nap_obj_b,_nr_bat],"AL_napalm\alias_buc_nap.sqf"] remoteExec ["execVM"];
