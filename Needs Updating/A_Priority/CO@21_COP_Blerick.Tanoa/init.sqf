...
Sleep 10;
	...
	if(isServer) then {
		[] spawn {
			{
				[west,_X,getPos ArtyTarget,4,300,120] spawn OKS_ArtyFire; sleep (random 60);
			} foreach [arty_1,arty_2];
		};
		[] spawn {
			{
				[west,_X,getPos ArtyTarget_1,4,300,120] spawn OKS_ArtyFire; sleep (random 60);
			} foreach [arty_3,arty_4];
		};
		[] spawn {
			{
				[west,_X,getPos ArtyTarget_2,4,300,120] spawn OKS_ArtyFire; sleep (random 60);
