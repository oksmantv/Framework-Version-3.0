...
switch (toLower(_role)) do {
	...
	case "sl": {
		...
		[[_grenade,2],[_rifle_mag_tr,5],[_smokegrenadeY,4],[_smokegrenadeB,3]] call _addToVest;
		[[_rifle_mag,4]] call _addToBackpack;
		_rangefinder call _addBino;
		[_map, "", "", _compass, _watch, ""] call _addLinkedItems;
		...
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			...
			((_loadout select 5) select 1) append [[["rhs_weap_rsp30_white","","","",[],[],""],10]];
		};
	};
	...
	case "ftl": {
		...
		[[_grenade,4],[_rifle_mag_tr,4],[_pistol_mag,2],[_smokegrenadeY,4]] call _addToVest;
		[[_rifle_mag,6]] call _addToBackpack;
		_rangefinder call _addBino;
		[_map, "", "", _compass, _watch, ""] call _addLinkedItems;
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			_nvg call _addNVG;
	...
	case "g": {
		...
		[[_grenade,4],[_rifle_mag,4],[_rifle_mag_tr,2]] call _addToVest;
		[[_clacker,1],[_demoCharge,2],[_rifle_mag_tr,2]] call _addToBackpack;
		[_map, _gps, "", _compass, _watch, ""] call _addLinkedItems;
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			_nvg call _addNVG;
			[[_mapFlashLight,1],[_IRStrobe,1]] call _addToUniform;
	...
	case "ab": {
		[_goggles,_helmet,_uniform,_vest,_backpack] call _addEquipment;
		[_rifleC, _rifleC_mag, ""] call _addPrimary;
		[[_cables,2],[_flashBang,2],[_rifleC_mag_tr,4],[_pistol_mag,2],[_smokegrenadeY,3]] call _addToUniform;
	...
	case "amat": {
		...
		[[_rifle_mag_tr,6]] call _addToVest;
		[[_HAT_mag,2],[_demoCharge,1],[_clacker,1]] call _addToBackpack;
		[_map, _gps, "", _compass, _watch, ""] call _addLinkedItems;
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			_nvg call _addNVG;
